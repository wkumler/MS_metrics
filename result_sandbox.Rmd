...
xcms_params <- c("mean_mz", "sd_ppm", "mean_rt", "sd_rt", "mean_pw", 
                 "sd_pw", "log_mean_height", "log_sd_height", "sn", 
...
interp_dt <- mapply(function(mzmed_i, rtmed_i, feat_id_i){
...
pc_features <- FT2040_features %>%
  select(feature=feat_id, feat_class) %>%
...
full_quick_model <- FT2040_features %>% 
  filter(feat_id%in%quick_classes$feature) %>%
  select(-feat_class) %>%
  left_join(quick_classes, by=c(feat_id="feature")) %>%
  select(-feat_id) %>%
