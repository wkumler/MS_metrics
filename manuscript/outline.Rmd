...
Include the descriptor for what "FTXXXX" means. 

Include the molecular mass in the feature label? If known, include name?

Put the actual good/bad label in the figure somewhere

What's the importance of the different colors?

Note that these were taken from the MESOSCOPE dataset.

Explain the chromatograms (RT/int/ what "goodness" requires?) explain x/y axis in figure legend, maybe do in bulk across all at once?

Explain that we grabbed the center of the peak +/- 1 (2?) minutes
...
Good figure, spacing between text and labels and data
...
## Discussion
  ...
  - Fundamental tradeoff between GPF and FDR should be determined by the scientific question
  - FDR and GPF rates are very rarely reported but critically important to include
  - Full model and XCMS model are definitely overfit
    - Steep performance drop on new data
    - Disagreement based on training set
  - Only tested on HILIC data, expectations for RP columns
  - Using medians/means as summary statistics ignores differences between treatments (i.e. lots of blanks blows up the median): maybe better to use max/top3? Maybe better to implement independently for each sample class?
  - XCMS alone does not provide enough information to distinguish good peaks from bad with much confidence
