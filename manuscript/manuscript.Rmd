...
fdr_gff_all_datasets_gp <- error_type_all_datasets %>%
  pivot_longer(c(`FDR`, `GFF`)) %>%
  drop_na() %>%
  mutate(desc=case_when(
    is.na(value) ~ NA,
    name=="FDR"~ paste0("frac(",`%FP`,",",`%FP`+`%TP`,")"),
    name=="GFF"~ paste0("frac(",`%TP`,",",`%FN`+`%TP`,")")
  )) %>%
  ...
  mutate(name=factor(name, levels=c("GFF", "FDR"),
                     labels=c("Fraction of good features found (GFF)", 
...
clockplot_complete_gp <- clockplot_base_gp +
  ...
  scale_shape_manual(breaks = c("Good MF found", "Bad MF included",
                                "Good MF missed", "Bad MF avoided"), 
                     values = c(19, 3, 4, 1))
...
![Supplemental figure 3: Performance of regularized regression and random forest models on internally (same train-test) and externally (different train-test) validated datasets.](supplement/regregrf_fdrgff_cross_train.png){width=90%}
