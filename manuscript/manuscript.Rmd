  ...
  imap(function(alpha_selection, alpha_name){
    lapply(dataset_versions, function(train_set){
      ...
      net_y <- get(train_set) %>% 
        ...
        mutate(feat_class=feat_class=="Good") %>%
        pull(feat_class)
      net_model <- cv.glmnet(x=net_x, y=net_y, family="binomial", alpha=alpha_selection)
...
rf_cross_train_df <- lapply(dataset_versions, function(train_set){
  ...
  rf_y <- get(train_set) %>% 
    ...
    pull(feat_class) %>%
    factor()
  rfmodel <- randomForest(rf_x, rf_y)
